...
TOKEN = '8926837188:AAEz9XRwLEVO3QLir6dXph4TAqvdliMrFl4'
OWNER_ID = 8695946179
ADMIN_ID = 8695946179
YOUR_USERNAME = '@sahilxalone'
UPDATE_CHANNEL = 'https://t.me/OSINTNXher'
...
# ✅ MONGODB CONFIG (replaces SQLite)
MONGO_URI = "mongodb+srv://develop10405:GuhDUZAKHzcPED4l@cluster0.s1quu.mongodb.net/?retryWrites=true&w=majority"
MONGO_DB_NAME = "mxhost"

BASE_DIR = os.path.abspath(os.path.dirname(__file__))
UPLOAD_BOTS_DIR = os.path.join(BASE_DIR, 'upload_bots')
IROTECH_DIR = os.path.join(BASE_DIR, 'inf')
# DATABASE_PATH kept only for folder creation compatibility (no sqlite file anymore)
...
def init_db():
    ...
    try:
        mongo_db.subscriptions.create_index('user_id', unique=True)
        mongo_db.user_files.create_index([('user_id', 1), ('file_name', 1)], unique=True)
        mongo_db.active_users.create_index('user_id', unique=True)
        mongo_db.admins.create_index('user_id', unique=True)
        mongo_db.file_approvals.create_index([('user_id', 1), ('file_name', 1)], unique=True)
        mongo_db.admins.update_one({'user_id': OWNER_ID}, {'$set': {'user_id': OWNER_ID}}, upsert=True)
        if ADMIN_ID != OWNER_ID:
            mongo_db.admins.update_one({'user_id': ADMIN_ID}, {'$set': {'user_id': ADMIN_ID}}, upsert=True)
...
def load_data():
    ...
    try:
        for doc in mongo_db.subscriptions.find():
            try:
                user_subscriptions[doc['user_id']] = {'expiry': datetime.fromisoformat(doc['expiry'])}
            except Exception:
                logger.warning(f"Invalid expiry format for user {doc['user_id']}: {doc.get('expiry')}. Skipping.")

        for doc in mongo_db.user_files.find():
            uid = doc['user_id']
            if uid not in user_files:
                user_files[uid] = []
            user_files[uid].append((doc['file_name'], doc['file_type']))

        active_users.update(doc['user_id'] for doc in mongo_db.active_users.find())
        admin_ids.update(doc['user_id'] for doc in mongo_db.admins.find())
...
# ✅ GRIDFS FILE STORAGE (replaces local-only file storage)
def save_file_bytes(user_id, file_name, data):
    """Store a user's file in MongoDB GridFS"""
    try:
        existing = mongo_db['fs.files'].find_one({'metadata.user_id': user_id, 'metadata.file_name': file_name})
        if existing:
            try:
                fs.delete(existing['_id'])
            except Exception:
                pass
        fs.put(data, filename=file_name, metadata={'user_id': user_id, 'file_name': file_name})
        logger.info(f"File stored in MongoDB: {user_id}/{file_name} ({len(data)} bytes)")
        return True
    except Exception as e:
        logger.error(f"Error storing file {user_id}/{file_name} in MongoDB: {e}", exc_info=True)
        return False

def get_file_bytes(user_id, file_name):
    """Read a user's file from MongoDB GridFS"""
    try:
        existing = mongo_db['fs.files'].find_one({'metadata.user_id': user_id, 'metadata.file_name': file_name})
        if not existing:
            return None
        return fs.get(existing['_id']).read()
    except Exception as e:
        logger.error(f"Error reading file {user_id}/{file_name} from MongoDB: {e}")
        return None

def delete_file_bytes(user_id, file_name):
    """Delete a user's file from MongoDB GridFS"""
    try:
        existing = mongo_db['fs.files'].find_one({'metadata.user_id': user_id, 'metadata.file_name': file_name})
        if existing:
            fs.delete(existing['_id'])
            logger.info(f"File deleted from MongoDB: {user_id}/{file_name}")
            return True
        return False
    except Exception as e:
        logger.error(f"Error deleting file {user_id}/{file_name} from MongoDB: {e}")
        return False

def ensure_local_file(user_id, file_name):
    """Download file from MongoDB to local disk if missing (VPS migration safe)"""
    user_folder = get_user_folder(user_id)
    local_path = os.path.join(user_folder, file_name)
    if os.path.exists(local_path):
        return local_path
    data = get_file_bytes(user_id, file_name)
    if data is None:
        return None
    try:
        with open(local_path, 'wb') as f:
            f.write(data)
        logger.info(f"Restored file from MongoDB to local disk: {local_path}")
        return local_path
    except Exception as e:
        logger.error(f"Error restoring file {local_path}: {e}")
        return None

def sync_all_files_from_mongo():
    """On startup, restore ALL user files from MongoDB to local disk"""
    logger.info("Syncing all user files from MongoDB to local disk...")
    restored = 0
    skipped = 0
    try:
        for doc in mongo_db['fs.files'].find():
            meta = doc.get('metadata') or {}
            uid = meta.get('user_id')
            fn = meta.get('file_name')
            if uid is None or not fn:
                continue
            user_folder = get_user_folder(uid)
            local_path = os.path.join(user_folder, fn)
            if os.path.exists(local_path):
                skipped += 1
                continue
            try:
                ...
                logger.error(f"Error restoring {uid}/{fn}: {e}")
        logger.info(f"File sync complete. Restored: {restored}, Already present: {skipped}")
    except Exception as e:
        logger.error(f"Error during file sync: {e}", exc_info=True)

# ✅ File approval functions (MongoDB)
def save_file_approval(user_id, file_name, file_type, status=FILE_STATUS_PENDING, reviewed_by=None, message_id=None):
    with DB_LOCK:
        try:
            ...
            mongo_db.file_approvals.update_one(
...
def get_file_status(user_id, file_name):
    try:
        doc = mongo_db.file_approvals.find_one({'user_id': user_id, 'file_name': file_name})
        if doc:
            return {
...
def update_file_status(user_id, file_name, status, admin_id):
    with DB_LOCK:
        try:
            ...
            result = mongo_db.file_approvals.update_one(
                {'user_id': user_id, 'file_name': file_name},
            ...
            if result.matched_count > 0:
                ...
                return True
            logger.warning(f"File approval record not found: {user_id}/{file_name}")
            return False
        except Exception as e:
...
def get_all_pending_files():
    try:
        cursor = mongo_db.file_approvals.find({'status': FILE_STATUS_PENDING}).sort('uploaded_time', -1)
        return [(d['user_id'], d['file_name'], d.get('file_type', 'unknown'), d.get('uploaded_time', '')) for d in cursor]
    except Exception as e:
...
def get_pending_files_count():
    try:
        return mongo_db.file_approvals.count_documents({'status': FILE_STATUS_PENDING})
    except Exception as e:
        logger.error(f"Error getting pending files count: {e}")
        return 0

# ✅ Send file for approval function
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    # ✅ Restore file from MongoDB if missing (VPS migration safe)
    restored_path = ensure_local_file(script_owner_id, file_name)
    if restored_path:
        script_path = restored_path

    # ✅ Check if file is approved before running
    ...
    try:
        if not os.path.exists(script_path):
             bot.reply_to(message_obj_for_reply, f"Error: Script '{file_name}' not found (also missing in MongoDB)!")
...
# ✅ MongoDB versions of DB helper functions
def save_user_file(user_id, file_name, file_type='py'):
    with DB_LOCK:
        try:
            mongo_db.user_files.update_one(
                {'user_id': user_id, 'file_name': file_name},
            ...
            logger.error(f"Error saving file for {user_id}, {file_name}: {e}", exc_info=True)

def remove_user_file_db(user_id, file_name):
    with DB_LOCK:
        try:
            mongo_db.user_files.delete_one({'user_id': user_id, 'file_name': file_name})
            if user_id in user_files:
                user_files[user_id] = [f for f in user_files[user_id] if f[0] != file_name]
                if not user_files[user_id]: del user_files[user_id]
            logger.info(f"Removed file '{file_name}' for user {user_id} from DB")
        except Exception as e:
            logger.error(f"Error removing file for {user_id}, {file_name}: {e}", exc_info=True)

def add_active_user(user_id):
    active_users.add(user_id)
    with DB_LOCK:
        try:
            mongo_db.active_users.update_one({'user_id': user_id}, {'$set': {'user_id': user_id}}, upsert=True)
    ...
    with DB_LOCK:
        try:
            ...
            mongo_db.subscriptions.update_one(
                {'user_id': user_id},
                {'$set': {'user_id': user_id, 'expiry': expiry_str}},
                upsert=True
            )
...
def remove_subscription_db(user_id):
    with DB_LOCK:
        try:
            mongo_db.subscriptions.delete_one({'user_id': user_id})
...
def remove_admin_db(admin_id):
    ...
    with DB_LOCK:
        try:
            result = mongo_db.admins.delete_one({'user_id': admin_id})
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        # ✅ NEW: Store ALL extracted files in MongoDB GridFS
        stored_count = 0
        for item_name in os.listdir(user_folder):
            item_path = os.path.join(user_folder, item_name)
            if os.path.isfile(item_path) and not item_name.endswith('.log'):
                try:
                    with open(item_path, 'rb') as fh:
                        if save_file_bytes(user_id, item_name, fh.read()):
                            stored_count += 1
                except Exception as e:
                    logger.error(f"Error storing {item_name} to MongoDB: {e}")
        logger.info(f"Stored {stored_count} files from zip into MongoDB")
        ...
        bot.reply_to(message,
                    ...
                    f"☁️ Stored in MongoDB: {stored_count} files\n"
...
def handle_js_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        ...
        # ✅ NEW: Store file in MongoDB GridFS
        with open(file_path, 'rb') as fh:
            save_file_bytes(script_owner_id, file_name, fh.read())
    ...
    for target_user_id, files_for_user in all_user_files_snapshot.items():
        ...
        for file_name, file_type in files_for_user:
            ...
            if not is_bot_running(target_user_id, file_name):
                # ✅ Restore from MongoDB if missing (VPS migration safe)
                file_path = ensure_local_file(target_user_id, file_name)
                if file_path and os.path.exists(file_path):
                    ...
                    logger.warning(f"File '{file_name}' for user {target_user_id} not found locally or in MongoDB. Skipping.")
...
@bot.message_handler(commands=['syncfiles'])
def handle_syncfiles_command(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "Admin permissions required.")
        return
    msg = bot.reply_to(message, "🔄 Syncing all files from MongoDB to local disk...")
    sync_all_files_from_mongo()
    bot.edit_message_text("✅ File sync complete! All files restored from MongoDB.", message.chat.id, msg.message_id)
...
def start_bot_callback(call):
    try:
        ...
        # ✅ Restore from MongoDB if missing
        restored_path = ensure_local_file(script_owner_id, file_name)
        if restored_path:
...
def delete_bot_callback(call):
    try:
        ...
        # ✅ Also remove from MongoDB GridFS + approvals
        delete_file_bytes(script_owner_id, file_name)
        with DB_LOCK:
            try:
                mongo_db.file_approvals.delete_one({'user_id': script_owner_id, 'file_name': file_name})
        ...
        try:
            bot.edit_message_text(
                f"Record `{file_name}` (User `{script_owner_id}`) and {deleted_str} deleted (from disk + MongoDB)!",
...
if __name__ == '__main__':
    ...
    # ✅ Restore all user files from MongoDB (VPS migration safe)
    sync_all_files_from_mongo()
    keep_alive()
